...
import { fetch, selectDevice } from './Actions/teams.js';
...
app.use('/sw.js', express.static(__dirname + '/static/service-worker.js'));
...
app.get('*', function (req, res) {
  match({ routes:Routes, location: req.url}, (error, redirectLocation, renderProps)=>{
    if (error) {
      ...
      store.dispatch(selectDevice( isCallerMobile(req) ));
        store.dispatch(fetch(renderProps.location.pathname)).then(() => {
            var element = (
                <Provider store={store}>
                    <RouterContext {...renderProps}  />
                </Provider>
            );
            const html = renderFullPage(renderToString(element), store.getState());
    ...
    if (e.code == 'EADDRINUSE') {
        ...
        clientSocket.on('error', function(e) { // handle error trying to talk to server
            if (e.code == 'ECONNREFUSED') {  // No other server listening
                ...
                server.listen(listen, function() { //'listening' listener
                    if(isNaN(listen)){
